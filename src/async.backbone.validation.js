Backbone.Validation = ((_ => {
    ...
    var Validation = ((() => {
        ...
        // validation rules.
        var getValidatedAttrs = model => _.reduce(_.keys(model.validation || {}), (memo, key) => {
            memo[key] = void 0;
            return memo;
        }, {});
        ...
        var getValidators = (model, attr) => {
            ...
            return _.reduce(attrValidationSet, (memo, attrValidation) => {
        ...
        var validateAttr = (masterDeferred, model, attr, value, computed) => {
        ...
        var validateModel = (model, attrs) => {
            var masterDeferred = $.Deferred();

            var error;
            var invalidAttrs = {};
            var isValid = true;
            var computed = _.clone(attrs);
            var flattened = flatten(attrs);

            // Convert an object into a list of [key, value] pairs.
            var pairs = _.pairs(flattened);

            var invokeModel = pair => {
                var attr = pair[0];
                var value = pair[1];
                var result = $.Deferred();
        ...
        var bindModel = (view, model, options) => {
            _.extend(model, mixin(view, options));
        };

        // Removes the methods added to a model
        var unbindModel = model => {
        ...
        return {
            ...
            bind(view, options) {
                ...
                if (model) {
                    ...
                    collection.bind('add', collectionAdd, {view, options});
    ...
    var defaultLabelFormatters = Validation.labelFormatters = {
        ...
        sentenceCase(attrName) {
            return attrName.replace(/(?:^\w|[A-Z]|\b\w)/g,(match, index) => index === 0 ? match.toUpperCase() : ' ' + match.toLowerCase()).replace('_', ' ');
        },
    ...
    var defaultValidators = Validation.validators = ((() => {
        // Use native trim when defined
        var trim = String.prototype.trim ?
            text => text === null ? '' : String.prototype.trim.call(text) :
            text => {
                var trimLeft = /^\s+/;
                var trimRight = /\s+$/;
        ...
        var isNumber = value => _.isNumber(value) || (_.isString(value) && value.match(defaultPatterns.number));

        // Determines whether or not not a value is empty
        var hasValue = value => !(_.isNull(value) || _.isUndefined(value) || (_.isString(value) && trim(value) === ''));

        return {
            // Function validator
            // Lets you implement a custom function used for validation
            fn(validator, value, attr, fn, model, computed) {
                if (_.isString(fn)) {
                    fn = model[fn];
                }
            ...
            required(validation, value, attr, required, model, computed) {
            ...
            acceptance(validation, value, attr, accept, model) {
            ...
            min(validation, value, attr, minValue, model) {
            ...
            max(validation, value, attr, maxValue, model) {
            ...
            range(validation, value, attr, range, model) {
            ...
            length(validation, value, attr, length, model) {
            ...
            minLength(validation, value, attr, minLength, model) {
            ...
            maxLength(validation, value, attr, maxLength, model) {
            ...
            // the two numbers specified
            rangeLength(validation, value, attr, range, model) {
            ...
            oneOf(validation, value, attr, values, model) {
            ...
            equalTo(validation, value, attr, equalTo, model, computed) {
            ...
            pattern(validation, value, attr, pattern, model) {
